...
class ImageToImage2D(Dataset):
    ...
    def __init__(self, dataset_path: str, split='train', joint_transform: Callable = None, img_size=256, max_tokens=25, max_query=9, one_hot_mask: int = False) -> None:
        ...
        self.tokenizer = BertTokenizer.from_pretrained(r"/home/lx/code/GMIRS/pretrained_weights/bert-base-uncased")
    ...
    def __getitem__(self, i):
        ...
        input_ids = self.tokenizer.encode(text=referring_txt, add_special_tokens=True) # encode=tokenize + convert_tokens_to_ids
        input_ids = input_ids[:self.max_tokens]
        padded_input_ids[:len(input_ids)] = input_ids
        attention_mask[:len(input_ids)] = [1] * len(input_ids)
        lang_tokens = torch.tensor(padded_input_ids)
        lang_masks = torch.tensor(attention_mask)
