...
from models.UniLSeg.utils.simple_tokenizer import SimpleTokenizer as _Tokenizer
...
class ImageToImage2D(Dataset):
    ...
    def __init__(self, dataset_path: str, split='train', joint_transform: Callable = None, img_size=256, max_tokens=17, one_hot_mask: int = False) -> None:
    ...
    def __getitem__(self, i):
        ...
        mask_instances = torch.zeros((self.max_tokens, *mask_shape))
        mask_merged = torch.zeros_like(mask)
    
        classes = str(class_index).split("-")
        class_num = len(classes)
        referring_existing = False
        for i in range(class_num):
            ...
            if torch.any(mask_i != 0):
                referring_existing = True
        
        # --------------------------------- Language data ---------------------------------
        lang_tokens = tokenize(referring_txt, self.max_tokens, True)
...
# ----------------------------------------- data transformer function from LViT -----------------------------------------
